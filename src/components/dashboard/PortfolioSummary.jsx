...
export default function PortfolioSummary({
  ...
  return (
    <div className="hero-card stagger-2">
      ...
      {hasPrices ? (
        <>
          ...
          {(totalCostUSD > 0 || initialCapitalUSD > 0 || totalRealizedUSD !== 0) && (
            <div
              className={`hero-pnl ${totalGainUSD >= 0 ? "up" : "down"}`}
        ...
        <div className="hero-meta-item" style={{ textAlign: "right" }}>
          ...
          >
            ...
            <span style={{ fontSize: 11, color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>
              ({fmt.thb(totalCostUSD * exchangeRate, 0)})
            </span>
          </span>
        </div>
      </div>
      <div
        className="hero-meta"
        style={{ marginTop: 10, borderTop: "1px dashed rgba(255,255,255,0.2)", paddingTop: 10 }}
      >
        <div className="hero-meta-item">
          ...
          >
            ...
            <span style={{ fontSize: 11, color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>
              ({totalRealizedUSD >= 0 ? "+" : ""}
              {fmt.thb(totalRealizedUSD * exchangeRate)})
            </span>
          </span>
        </div>
        <div className="hero-meta-item" style={{ textAlign: "right" }}>
          ...
          >
            ...
            <span style={{ fontSize: 11, color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>
              ({totalUnrealizedUSD >= 0 ? "+" : ""}
              {fmt.thb(totalUnrealizedUSD * exchangeRate)})
            </span>
          </span>
        </div>
      </div>
      ...
      {hasPrices && todayChangeUSD !== 0 && (
        ...
        >
          ...
          <span style={{ fontSize: 14, fontWeight: 800, color: todayChangeUSD >= 0 ? "#6EE7B7" : "#FCA5A5" }}>
            {todayChangeUSD >= 0 ? "+" : ""}
            {fmt.usd(todayChangeUSD)}{" "}
            <span style={{ fontSize: 11, opacity: 0.85, fontWeight: 700 }}>
              ({todayChangeUSD >= 0 ? "+" : ""}
              {fmt.thb(todayChangeUSD * exchangeRate, 0)})
            </span>
          </span>
        </div>
      )}
