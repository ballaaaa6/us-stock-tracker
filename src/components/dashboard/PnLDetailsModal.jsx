...
export default function PnLDetailsModal({
  ...
  const filtered = useMemo(() => {
    ...
    return breakdown.filter((b) => b.symbol.toLowerCase().includes(q) || b.name.toLowerCase().includes(q));
      ...
      <div className="modal-content" style={{ maxWidth: 840, width: "95%" }}>
        <div className="modal-header" style={{ borderBottom: "1px solid var(--border)", paddingBottom: 14 }}>
          <span className="modal-title" style={{ fontSize: 16, fontWeight: 800 }}>
            📊 รายละเอียดกำไร/ขาดทุนรายสินทรัพย์ (P&L Breakdown)
          </span>
          <button
            className="btn-close"
            onClick={onClose}
        ...
        >
          ...
          <div>
            ...
            <div
              style={{ fontSize: 15, fontWeight: 800, color: totalRealizedUSD >= 0 ? "var(--gain)" : "var(--loss)" }}
            >
              {totalRealizedUSD >= 0 ? "+" : ""}
              {fmt.usd(totalRealizedUSD)}
            </div>
            <div style={{ fontSize: 11, color: "var(--text-faint)" }}>
              {totalRealizedTHB_Modal >= 0 ? "+" : ""}
              {fmt.thb(totalRealizedTHB_Modal)}
            </div>
          </div>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", marginBottom: 4 }}>
              ยังไม่รับรู้ (Unrealized)
            </div>
            <div
              style={{ fontSize: 15, fontWeight: 800, color: totalUnrealizedUSD >= 0 ? "var(--gain)" : "var(--loss)" }}
            >
              {totalUnrealizedUSD >= 0 ? "+" : ""}
              {fmt.usd(totalUnrealizedUSD)}
            </div>
            <div style={{ fontSize: 11, color: "var(--text-faint)" }}>
              {totalUnrealizedTHB_Modal >= 0 ? "+" : ""}
              {fmt.thb(totalUnrealizedTHB_Modal)}
            </div>
          </div>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", marginBottom: 4 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800, color: totalGainUSD >= 0 ? "var(--gain)" : "var(--loss)" }}>
              {totalGainUSD >= 0 ? "+" : ""}
              {fmt.usd(totalGainUSD)}
            </div>
            <div
              style={{
                fontSize: 11,
                color: totalGainTHB_Modal >= 0 ? "var(--gain)" : "var(--loss)",
                fontWeight: 700,
                display: "flex",
            ...
            >
              <div>
                {totalGainTHB_Modal >= 0 ? "+" : ""}
                {fmt.thb(totalGainTHB_Modal)}
              </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, textAlign: "left" }}>
            <thead>
              <tr style={{ background: "#F1F5F9", position: "sticky", top: 0, zIndex: 1 }}>
                <th style={{ padding: "10px 12px", fontWeight: 700, color: "var(--text-muted)", width: "16%" }}>
                  สินทรัพย์
                </th>
                <th
                  style={{
                    ...
                    >
                      <td style={{ padding: "10px 12px", fontWeight: 700 }}>
                        <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
                          ...
                          <span
                            className={`badge-type ${item.category || "stock"}`}
                            style={{ fontSize: 9, padding: "1px 4px", borderRadius: 4 }}
                          >
                            {item.category === "gold"
                              ? item.symbol === "CL=F"
                                ? "น้ำมัน"
                      ...
                      <td
                        style={{
                          ...
                          textAlign: "right",
                          fontWeight: 700,
                          color: item.realized >= 0 ? "var(--gain)" : "var(--loss)"
                        }}
                      >
                        <div>
                        ...
                        {item.realized !== 0 && (
                          <div
                            style={{
                              fontSize: 10,
                              color: item.realizedTHB >= 0 ? "var(--gain)" : "var(--loss)",
                              fontWeight: "normal"
                            }}
                          >
                            ({item.realizedTHB >= 0 ? "+" : ""}
                            {fmt.thb(item.realizedTHB)})
                          </div>
                        )}
                      </td>
                      <td
                        style={{
                          ...
                          textAlign: "right",
                          fontWeight: 700,
                          color: item.unrealized >= 0 ? "var(--gain)" : "var(--loss)"
                        }}
                      >
                        <div>
                          {item.unrealized !== 0 && !isSoldOut
                            ? (item.unrealized >= 0 ? "+" : "") + fmt.usd(item.unrealized)
                            : "—"}
                        </div>
                        {item.unrealized !== 0 && !isSoldOut && (
                          <div
                          ...
                          >
                            ({item.unrealized >= 0 ? "+" : ""}
                            {fmt.thb(item.unrealized * exchangeRate)})
                          </div>
                        )}
                      </td>
                      <td
                        style={{
                          ...
                          textAlign: "right",
                          fontWeight: 800,
                          color: item.totalPnL >= 0 ? "var(--gain)" : "var(--loss)"
                        }}
                      >
                        <div>
                          {item.totalPnL >= 0 ? "+" : ""} {fmt.usd(item.totalPnL)}
                        </div>
                        <div
                          style={{
                            fontSize: 10,
                            color: totalPnLTHB >= 0 ? "var(--gain)" : "var(--loss)",
                            fontWeight: "bold"
                          }}
                        >
                          ({totalPnLTHB >= 0 ? "+" : ""}
